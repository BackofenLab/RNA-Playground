...
function FornaContainer(element, passedOptions) {
    ...
    self.centerView = function() {
        ...
        minX = d3.min(self.graph.nodes.map(function(d) {return d.x-d.radius;}));
        minY = d3.min(self.graph.nodes.map(function(d) {return d.y-d.radius;}));

        maxX = d3.max(self.graph.nodes.map(function(d) {return d.x+d.radius;}));
        maxY = d3.max(self.graph.nodes.map(function(d) {return d.y+d.radius;}));
