...
(function () {  // namespace
    ...
    function updateDocumentView(algorithm, view) {
        ...
        view.load(PATHS.MAIN.PAGES + algorithm + FILE_EXTENSIONS.HYPERTEXT_MARKUP_LANGUAGE, function () {
            var container = $("#algorithm_interface");

            switch (algorithmName) {
                case ALGORITHMS.GOTOH:
                    load(container, algorithmName, PATHS.HTML.AFFINE_ALIGNMENT_INTERFACE);
                    break;
                case ALGORITHMS.NEEDLEMAN_WUNSCH:
                    load(container, algorithmName, PATHS.HTML.ALIGNMENT_INTERFACE);
                    break;
    ...
    /**
     * Loads the interface of an algorithm.
     * @param container - The container (div) in which you load the content.
     * @param algorithmName - The algorithm which you want to use.
     * @param path - The path of the interface you want load in the container.
     */
    function load(container, algorithmName, path) {
        container.load(path, function () {  // ".js" executed when html page has fully loaded
